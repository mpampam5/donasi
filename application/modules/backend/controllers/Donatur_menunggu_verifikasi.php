...
/* modules/backend/controllers/Donatur_menunggu_verifikasi.php */
...
class Donatur_menunggu_verifikasi extends Backend{
	...
	public function __construct()
  {
    ...
    $this->load->model("Donatur_menunggu_verifikasi_model","model");
  }
	...
	function index()
  {
    $this->temp_backend->set_title('Donatur Menunggu Verifikasi');
    $this->temp_backend->view('content/donatur_menunggu_verifikasi/index');
  }
	...
	function detail($id)
  {
    if ($row = $this->model->get_where($this->table,['id_donatur'=>$id])) {
      ...
      $this->temp_backend->view('content/donatur_menunggu_verifikasi/detail',$data);
    }else {
      $this->_error404();
    }
  }
...
function add()
{
    $this->temp_backend->set_title('Donatur Menunggu Verifikasi');
      $data = [
                'button' => 'tambah',
                'action' => site_url('backend/donatur_menunggu_verifikasi/add_action'),
								'nama'	=>	set_value('nama'),
    ...
    $this->temp_backend->view('content/donatur_menunggu_verifikasi/form',$data);
}
...
function update($id)
{
  if ($row = $this->model->get_where($this->table,['id_donatur'=>$id])) {
    $this->temp_backend->set_title('Donatur Menunggu Verifikasi');
      ...
      $data = [
                'button' => 'edit',
                'action' => site_url('backend/donatur_menunggu_verifikasi/update_action/'.$id),
...
} //End Class Donatur_menunggu_verifikasi
